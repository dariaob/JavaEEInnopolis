CREATE TABLE IF NOT EXISTS office(                    -- Данные кабинета приема
                                     "id" bigint primary key not null,                 -- Номер(id) кабинета
                                     "office_type" varchar(255)                        -- Назначение кабинета
);

CREATE TABLE IF NOT EXISTS patient_card(              -- Карта больного
...
CREATE TABLE IF NOT EXISTS doctor(                    -- Информация по врачу
...
CREATE TABLE IF NOT EXISTS patient(                   -- Данные о пациенте
